...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        # when looping through parents, keep a list of parents visited 
        # and take the parent which occurs at the lowest index of one
        # list and use it as the ancestor instead of root
        dist_root_a = 0
        dist_root_b = 0
        if a == self.root:
            dist_root_a = -1
        ...
        if cous_type != -1:
            cous_type = min(dist_root_a, dist_root_b)            

        deg_removed = abs(dist_root_b - dist_root_a)
...
assert words[t] == words[1]
assert r == 0

t, r = f.cousin("i", "n")
print "'i' is a", words[t],"cousin", r, "removed from 'n'"
assert words[t] == words[2]
assert r == 0
